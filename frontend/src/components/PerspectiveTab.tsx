...
const markdownComponents: Components = {
  ...
  h3: ({ children }) => (
    <h3 className="text-sm font-bold text-t-green mt-3 mb-2">{children}</h3>
  ),
  p: ({ children }) => (
    <p className="text-t-green-dim leading-relaxed mb-2">{children}</p>
  ),
  strong: ({ children }) => (
  ...
  blockquote: ({ children }) => (
    <blockquote className="border-l-2 border-t-red pl-4 py-2 my-2 bg-t-red/5 text-t-green-dim">
      {children}
    </blockquote>
  ),
  table: ({ children }) => (
  ...
  td: ({ children }) => (
    <td className="py-1 px-2 text-t-green-dim border-b border-t-border/30">{children}</td>
  ),
  a: ({ href, children }) => (
              ...
              <Collapsible.Trigger asChild>
                <button className="w-full flex items-center justify-between py-2 px-3 border border-t-border bg-t-dark hover:bg-t-gray transition-colors text-left">
                  <div className="flex items-center gap-3">
                    <span className="text-t-green text-xs">{isOpen ? "v" : ">"}</span>
                    <span className="text-sm font-bold text-t-green-dim">
                      {verdict?.persona_name || `Persona ${i + 1}`}
                    </span>
                    {verdict?.persona_label && (
                      <span className="text-[10px] text-t-dim">{verdict.persona_label}</span>
                    )}
                  </div>
